...
export const importArchive = async (
  ...
  if (!isWorkspacePayload(parsed)) return rejected("The archive's workspace definition is not valid.");
  if (!entityCountsWithinBounds(parsed)) return rejected('The archive contains more entities than can be imported.');
  ...
  const remapped = remapWorkspaceIds(parsed);
  ...
  const oldRelationIds = new Map(
    Object.values(parsed.datasets).map((dataset) => [dataset.id, dataset.relationId] as const),
  );
  const oldByPosition = Object.values(parsed.datasets);
